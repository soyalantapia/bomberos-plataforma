...
function fallback(r: Rendicion): ItemCopilotoRendicion[] {
  return r.requisitos
    ...
    .map((req) => ({
      ...
      acciones: [
        req.linkPagina
          ? `Resolvelo en ${req.linkPagina.replace(/^\//, '')}`
          : 'Completalo desde la página correspondiente',
        'Volvé a generar el paquete cuando esté',
      ],
      importanciaTexto:
        req.importanciaTexto ?? 'Es un requisito formal del Fondo para liquidar el subsidio.',
      textoRedactado:
        req.id === 'req-firmas'
...
const SYSTEM = `Sos el copiloto de Vulcano, plataforma para bomberos voluntarios argentinos.
