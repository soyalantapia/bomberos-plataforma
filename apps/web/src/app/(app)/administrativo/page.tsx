...
export default function PadronPage() {
  ...
  const personas = useMemo(() => allPersonas.filter((p) => p.cuartelId === cuartel?.id), [allPersonas, cuartel?.id]);

  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      <SectionHeader title="Padrón / Legajos" description={`${personas.length} personas · ficha única que reemplaza los 28 formularios de GIB`} />
            ...
            {personas.map((p) => (
              <li key={p.id} className="p-3 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-slate-600">{p.legajo} · {fmtJerarquia(p.jerarquia)}</div>
                </div>
