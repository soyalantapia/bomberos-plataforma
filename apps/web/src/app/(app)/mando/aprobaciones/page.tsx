  ...
  ascensos: [
    {
      ...
      tipo: 'Pliego de ascenso',
      detalle: 'A Sargento · cumple antigüedad y cursos',
      fecha: '10/5/2026',
      estado: 'pendiente',
    },
  ],
...
export default function AprobacionesPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      <SectionHeader
        title="Aprobaciones"
        description="Decisiones explícitas con doble check y audit log"
      />
                          ...
                          {s.estado === 'pendiente' ? (
                            <div className="flex gap-2">
                              <Button
                                intent="success"
                                size="sm"
                                onClick={() => decidir(lista, s.id, 'aprobar')}
                              >
                              ...
                              <Button
                                intent="ghost"
                                size="sm"
                                onClick={() => decidir(lista, s.id, 'rechazar')}
                              >
                                <X size={16} /> Rechazar
                              </Button>
                            </div>
                          ) : (
                            <Badge intent={s.estado === 'aprobada' ? 'ok' : 'risk'}>
                              {s.estado}
                            </Badge>
                          )}
                        </li>
